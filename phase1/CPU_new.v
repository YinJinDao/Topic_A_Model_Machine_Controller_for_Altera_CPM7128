// ============================================================
// 文件名: CPU_new.v  ——  TEC-8 模型机硬布线顺序控制器
...
//
// 设计说明:
//   - 时序逻辑控制器，输出在 W1/W2/W3 上升沿同步更新
//   - CLR# 低电平时所有控制信号清零
//   - STO 内部状态寄存器在时序块中统一管理（已合并 SSTO）
...
module CPU_new (
    ...
    input  wire       T3,         // 节拍脉冲 T3，pin 83（保留用于兼容）
    ...
    //       CLR# 后 STO=0；在时序块中根据需要设为 1
    reg  STO;

    // 统一时钟：任一 W 信号的上升沿触发（确保单时钟可综合）
    wire w_any = W1 | W2 | W3;

    // ============================
    // 主时序逻辑
    //
    // 敏感列表：w_any 上升沿 + CLR 下降沿
    // 输出在 W 信号上升沿同步更新，消除组合逻辑毛刺
    // 每个边沿触发时先清零所有输出（STO 除外），再按需置位
    // ============================

    always @(posedge w_any or negedge CLR) begin
        if (!CLR) begin
            // ---- 复位：所有输出 + STO 清零 ----
            MEMW  <= 1'b0;
            STOP  <= 1'b0;
                ...
                case (SW)
                    ...
                    3'b000: begin
                        case (opcode)
                            ...
                            4'b1010: begin
                                ...
                                S2    <= 1'b1;
                                S1    <= 1'b0;
                                S0    <= 1'b0;   // S=1010（传递 Rs 到总线）
                                ABUS  <= 1'b1;
                                // 将数据通过 ALU→ABUS→DBUS 供显示
                            end
                    ...
                    3'b100: begin
                        if (!STO) begin
                            ...
                            SEL2  <= 1'b1;
                            SEL1  <= 1'b1;
                            SEL0  <= 1'b0;   // SEL=1110（修正）
                            SBUS  <= 1'b1;
                            SELCTL<= 1'b1;
                            DRW   <= 1'b1;
                            STOP  <= 1'b1;
                            STO   <= 1'b0;   // 恢复 STO，下次回到 R0
                        end
                    end
